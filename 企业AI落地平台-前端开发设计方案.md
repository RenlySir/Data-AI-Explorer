...
| `/admin/datasources` | 数据源管理 | 搜索筛选、连接状态、添加、测试、删除、进入问数 |
...
数据源管理采用独立一级页面。顶部是可用/数据库/文件统计和唯一主操作“添加数据源”；列表支持名称、数据库、主机搜索及数据库/文件筛选；每项展示连接状态、地址、结构对象数或文件行数。数据库向导为类型 → 名称 → 主机/端口 → 数据库/账号/密码 → 保存并测试；文件向导为 CSV/Parquet 选择 → 解析 → 注册。连接失败在卡片内保留原因和重试，删除使用二次确认，演示源不可删除。页面不回显已保存密钥。

“进入问数”把数据源 ID 写入临时页面偏好并跳转 ChatBI；ChatBI 初始化时优先恢复该 ID。数据源管理是唯一的配置入口，任何新增、状态变化和删除均由该页操作并重新请求服务端结果同步；ChatBI 只读取统一注册表供顶部选择器使用，禁止复制一份本地数据源清单。
...
| 数据源管理 | admin-datasource | 添加数据源/进入问数 |
...
智能问数在侧栏作为可展开一级导航，纵向展示“ChatBI”和“大屏展示”两个二级项；点击一级项默认进入 ChatBI，二级项与页面视图共享选中状态。页面内部仍保留同样的两个 Tab，便于用户在内容区切换。ChatBI 聊天框顶部保留数据源选择器，选项只来自统一服务端注册表；页面不提供添加、上传、测试和删除数据源的操作。平台管理下独立“数据源管理”页面承担统一搜索、筛选、状态、添加、上传、测试和删除，密码框不回填、不显示已保存值。
