...
当前版本已包含可运行的 Vite 前端、FastAPI 智能问数、AIOps SQL 优化、多场景中心和知识库联调切片。“功能中心”作为 8 个模块、63 项功能的统一操作入口；首次登录会引导接入公有或私有大模型。平台管理提供独立数据源管理页，已打通 TiDB/MySQL 手动添加、CSV/Parquet 上传、连接测试、删除以及带选中数据源进入 ChatBI。生产凭证库、向量库、认证权限和持久化仍需按专项测试报告完成集成验收。
