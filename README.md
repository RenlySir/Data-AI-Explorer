...
| 18 | [模块与功能详细设计方案](企业AI落地平台-模块与功能详细设计方案.md) | 8 个一级模块、60 项功能、输入产出、门禁、状态与代码追踪 |
...
当前版本已包含可运行的 Vite 前端、FastAPI 智能问数、AIOps SQL 优化、多场景中心和知识库联调切片。新增“功能中心”作为 8 个模块、60 项功能的统一操作入口，支持按角色、交付状态和关键词筛选，并区分可使用、演示闭环与待生产接入。真实外部 Adapter、向量库、模型网关、认证权限和持久化仍需按专项测试报告完成生产集成验收。
