...
一级导航“知识库”采用单页任务流：知识库列表、概览、检索问答、引用、文档和资料入库。切库清空回答；创建、文本、上传、目录和查询使用独立 busy 状态。当前实现位于 apps/web/src/main.tsx 与 apps/web/src/knowledge.css，生产版再拆路由、缓存、分片上传、SSE 索引进度和原文定位。
