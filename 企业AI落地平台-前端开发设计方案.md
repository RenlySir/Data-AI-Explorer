...
“进入问数”把数据源 ID 写入临时页面偏好并跳转 ChatBI；ChatBI 初始化时优先恢复该 ID。独立页面与智能问数内的快捷添加共用服务端注册表，任何新增、状态变化和删除均由重新请求服务端结果同步，禁止复制一份本地数据源清单。
...
| 数据源管理 | admin-datasource、query-datasources | 添加数据源/进入问数 |
...
智能问数页以内嵌 Tab 组织“数据源、ChatBI、大屏展示”，用于分析过程中的快捷切换；平台管理同时提供独立“数据源管理”页面，承担统一搜索、筛选、状态、测试和删除。两处共享同一 API 和注册表。内嵌数据源页左侧解释接入边界，右侧列出状态、结构数量和连接测试；添加按钮打开 TiDB/MySQL 表单，文件入口直接上传 CSV/Parquet。密码框不回填、不显示已保存值。
