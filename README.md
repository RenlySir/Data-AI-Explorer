...
当前版本已包含可运行的 Vite 前端、FastAPI 智能问数、AIOps SQL 优化、多场景中心和知识库联调切片。“功能中心”作为 8 个模块、63 项功能的统一操作入口；首次登录会引导接入公有或私有大模型，已打通模型登记、连接测试、默认模型以及 Text2SQL/图表推理调用。生产凭证库、向量库、认证权限和持久化仍需按专项测试报告完成集成验收。
