...
本次实现已完成“添加数据源 → 数据关系采集 → ChatBI 提问 → Text2SQL → 数据库/文件执行 → ECharts → 用户认可 → 大屏复用”的可联调闭环。FastAPI 全量自动化 36/36 通过，前端构建、Python 编译、Compose 配置和浏览器主流程均通过。真实 TiDB 直连已读取对象及字段 Comment；数据关系支持外键、SQL JOIN 推断和页面关闭后仍运行的服务端持续采集任务；区域 GMV 问题生成 JOIN SQL 并自动选择饼图；CSV/DuckDB 等原有链路保持通过。
...
| UI-003 | 数据关系页 | 选择数据源、Schema/表/Comment、表级/字段级网络和 SQL 采集 | PASS |
...
后端命令：在 `backend` 目录执行 `python -m unittest discover -s tests -p 'test_*.py'`。本次结果：`Ran 35 tests`、`OK`。覆盖数据关系、数据源、凭证脱敏、ChatBI、危险意图、大屏创建/查询/移除。
...
2. 数据关系页显示 2 个 Schema、3 张表；网络图实际渲染表/字段节点与元数据/SQL 连线；展开 `sales.orders` 可见 `order_id` Comment“订单唯一标识”。
...
| 独立数据源管理页 | PASS | 数据源只在平台管理入口添加/上传/测试/删除；智能问数无重复管理入口；“进入问数”和 ChatBI 顶部选择器可恢复/切换数据源；桌面与 390px 移动端无横向溢出 |
| FastAPI 全量回归 | PASS | 36/36，包含 `test_chatbi.py` 与 `test_data_relationships.py` |
| 数据关系回归 | PASS | 36/36，新增元数据网络、手工 SQL JOIN 字段关系、服务端采集启停、TiDB 摘要重复采集去重与非 SELECT 拒绝 |
